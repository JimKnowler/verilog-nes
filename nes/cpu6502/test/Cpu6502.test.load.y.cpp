...
TEST_F(Cpu6502, ShouldImplementLDYi) {
    ...
    Trace expected = TraceBuilder()
        ...
        .port(o_address).signal({0, 1, 2, 3})
                        .repeatEachStep(2)
        .port(o_debug_y).signal({0x00, kTestData})
                        .repeatEachStep(4)
...
TEST_F(Cpu6502, ShouldImplementLDYzeropageIndexedWithX) {
    sram.clear(0);
    ...
    Trace expected = TraceBuilder()
        ...
        .port(o_address).signal({
                            ...
                            5})
                        .repeatEachStep(2)
        .port(o_debug_y).signal({0x00}).repeat(5)
                        .signal({kTestData})
...
TEST_F(Cpu6502, ShouldImplementLDYzeropageIndexedWithXProcessorStatus) {
    const uint8_t kTestAddress = 0x78;
