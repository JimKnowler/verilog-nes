...
TEST_F(Cpu6502, ShouldImplementANDZeroPageIndirectIndexedWithX) {
    sram.clear(0);
    ...
    assembler
        ...
        .label("init")
            ...
            .LDA().immediate(kTestPreloadA)
        .label("start")
            .AND().zpIndirect(kTestAddressZeroPage).x()
            .NOP()
        .org(0xFFFC)
        .word("init")
        .compileTo(sram);
...
TEST_F(Cpu6502, ShouldImplementANDZeroPageIndirectIndexedWithXProcessorStatus) {
    for (auto& testCase : kTestCasesAND) {
        const uint8_t kTestData1 = testCase.first.first;
        const uint8_t kTestData2 = testCase.first.second;
...
TEST_F(Cpu6502, ShouldImplementEORZeroPageIndirectIndexedWithX) {
    sram.clear(0);
    ...
    assembler
        ...
        .label("init")
            ...
            .LDA().immediate(kTestPreloadA)
        .label("start")
            .EOR().zpIndirect(kTestAddressZeroPage).x()
            .NOP()
        .org(0xFFFC)
        .word("init")
        .compileTo(sram);
...
TEST_F(Cpu6502, ShouldImplementEORZeroPageIndirectIndexedWithXProcessorStatus) {
    for (auto& testCase : kTestCasesEOR) {
        const uint8_t kTestData1 = testCase.first.first;
        const uint8_t kTestData2 = testCase.first.second;
